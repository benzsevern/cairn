...
- **slug**: lowercase kebab-case, no articles ("the", "a"), singular when possible. If an `<existing-concepts>` entry already describes this concept, REUSE its exact slug.
  ...
  - **If a slug already appears in `<existing-concepts>` (or in `slug_reuse_context` implied by it), `kind` MUST be `refined` or `referenced` — NEVER `introduced`.** Re-introducing an existing slug is a correctness bug.
